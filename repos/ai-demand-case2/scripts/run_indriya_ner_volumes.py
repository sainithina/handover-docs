#!/usr/bin/env python3
"""
NER + noun-phrase chunking with importance_score >= 0.1 threshold for Indriya prompts.

Reads prompts from indriya-prompts-20260728.xlsx (sheet "Prompt – Performing Prompts"),
runs the same experimental NER pipeline used in test_wylie_hotel_all_methods.py,
filters keywords at threshold 0.1 (fallback: keep top-scoring keyword if none survive),
fetches SV/ASV via DataForSEO for India (location_code=2356), fuses with Case2Estimator,
and writes results to Excel.

Usage:
    PYTHONPATH=src python scripts/run_indriya_ner_volumes.py
"""
...
INPUT_PATH = Path("/Users/sainithinartham/Downloads/indriya-prompts-20260728.xlsx")
INPUT_SHEET = "Prompt – Performing Prompts"
OUTPUT_DIR = REPO_ROOT / "runs" / "indriya_ner_th0.1"
OUTPUT_PATH = OUTPUT_DIR / "indriya-prompts-20260728-ner-volumes.xlsx"
OUTPUT_PATH_DOWNLOADS = Path("/Users/sainithinartham/Downloads/indriya-prompts-20260728-ner-volumes.xlsx")
    ...
    *,
    ...
    for p in prompts:
        ...
        scored = score_keywords_with_importance(
            prompt, candidate_texts, model_name=cross_encoder_model, max_keywords=max_keywords,
        )
...
def apply_threshold(
    ner_results: dict[str, list[tuple[str, float, str, str]]],
    threshold: float,
) -> tuple[dict[str, list[tuple[str, float, str, str, bool]]], dict[str, bool]]:
    """Filter per prompt; returns entries with kept_via_fallback flag."""
    filtered: dict[str, list[tuple[str, float, str, str, bool]]] = {}
...
def estimate_rows(
    prompts: list[dict],
    per_prompt_keywords: dict[str, list[tuple[str, float, str, str, bool]]],
    *,
    hp: Case2Hyperparameters,
    rho_by_keyword: dict,
    ...
    for p in prompts:
        ...
        for ke in kw_estimates:
            ...
            rows.append({
                ...
                "prompt_id": pid,
                "Prompt": prompt,
                "source_Volume": meta.get("Volume", ""),
                "source_Presence": meta.get("Presence", ""),
                "source_Funnel": meta.get("Funnel", ""),
                ...
                "extraction_type": extra[2] if len(extra) > 2 else "",
                "spacy_label": extra[3] if len(extra) > 3 else "",
...
def load_prompts() -> tuple[list[dict], pd.DataFrame, dict[str, dict]]:
    df = pd.read_excel(INPUT_PATH, sheet_name=INPUT_SHEET)
    prompts: list[dict] = []
    source_meta: dict[str, dict] = {}
    for i, row in df.iterrows():
        pid = f"indriya_{i + 1:04d}"
...
def build_summary(prompts: list[dict], detail_df: pd.DataFrame, source_meta: dict[str, dict]) -> pd.DataFrame:
    rows = []
    for p in prompts:
        pid = p["prompt_id"]
        ...
        row = {
            ...
            "source_Presence": meta.get("Presence", ""),
            "source_Funnel": meta.get("Funnel", ""),
            "source_Performance": meta.get("Performance", ""),
            "ner_th_0.1_y_median": float(sub["prompt_ai_demand_median"].iloc[0]) if len(sub) else None,
            "ner_th_0.1_y_mean": float(sub["prompt_ai_demand_mean"].iloc[0]) if len(sub) else None,
            "ner_th_0.1_keyword_count": len(sub),
        ...
        if len(sub):
            ...
            row["ner_th_0.1_top_keyword"] = sub.loc[top_idx, "keyword"]
            row["ner_th_0.1_top_keyword_sv"] = sub.loc[top_idx, "sv"]
            row["ner_th_0.1_top_keyword_asv"] = sub.loc[top_idx, "asv"]
        rows.append(row)
...
async def main() -> None:
    ...
    if not REFERENCE_CALIBRATION.exists():
        print(f"ERROR: calibration not found: {REFERENCE_CALIBRATION}", file=sys.stderr)
        sys.exit(1)
    ...
    prompts, source_df, source_meta = load_prompts()
    print(f"[load] {len(prompts)} prompts from {INPUT_PATH.name}")

    print(f"[setup] loading spaCy {SPACY_MODEL} ...")
    _get_nlp()
    ...
    print(f"[ner] extracting + scoring keywords for {len(prompts)} prompts ...")
    ner_results, extraction_log = run_ner_extraction(
        prompts, max_keywords=max_keywords, cross_encoder_model=cross_encoder_model,
    )

    print(f"[filter] applying importance threshold >= {IMPORTANCE_THRESHOLD} ...")
    filtered_results, fallback_flags = apply_threshold(ner_results, IMPORTANCE_THRESHOLD)
    n_fallback = sum(1 for v in fallback_flags.values() if v)
    print(f"    -> {n_fallback} prompts needed fallback (0 survivors at threshold)")

    all_keywords: set[str] = set()
    for entries in filtered_results.values():
        all_keywords.update(e[0] for e in entries)
    all_keywords_list = sorted(all_keywords)
    print(f"[sv/asv] fetching for {len(all_keywords_list)} unique keywords ...")
    ...
    print("[estimate] running Case2Estimator ...")
    detail_rows = estimate_rows(
        prompts, filtered_results,
        hp=hp, rho_by_keyword=rho_by_keyword,
        sv_map=sv_map, asv_map=asv_map, cpc_map=cpc_map, comp_map=comp_map,
        source_meta=source_meta,
    )
    detail_df = pd.DataFrame(detail_rows)
    summary_df = build_summary(prompts, detail_df, source_meta)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    for out_path in (OUTPUT_PATH, OUTPUT_PATH_DOWNLOADS):
        with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
            summary_df.to_excel(writer, sheet_name="ner_th_0.1_summary", index=False)
            detail_df.to_excel(writer, sheet_name="ner_th_0.1", index=False)
    ...
    print(f"\nMean Y_median (ner th=0.1): {summary_df['ner_th_0.1_y_median'].mean():.2f}")
    print(f"Median Y_median: {summary_df['ner_th_0.1_y_median'].median():.2f}")
    print("\nTop 10 prompts by Y_median:")
    print(summary_df.nlargest(10, "ner_th_0.1_y_median")[["Prompt", "ner_th_0.1_y_median", "ner_th_0.1_keyword_count"]].to_string(index=False))
